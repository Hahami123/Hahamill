...
def choose_product(call):
    ...
    if call.data == 'Egg_gotovo':
        ...
        btn_nazad = types.InlineKeyboard('Назад', calllback_date = 'back')
        markup3.add(btn_nazad)
        #открываем картинку 
        file = open('яичница', 'rb') 
        #отправляем картинку 
        bot.send_photo(call.message.chat.id, file = open('яичница.jpg', 'rb'))
        bot.send_message(call.message.chat.id, 'орпасшнс', reply_markup=markup3)
...
def back_to_products(call):
    # Здесь напишите логику возврата к выбору продуктов
    pass
